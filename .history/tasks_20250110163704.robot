...
${OUTPUT_DIR}    C:\\Users\\Ponraj\\OneDrive - Yitro Business Consultants India Private Limited\\Pictures\\Saved Pictures  overwrite=True
...
collect the receipt
   ...
   # Step 2: Export the receipt as a PDF
    ${pdf_path}=    Create Path    ${OUTPUT_DIR}${/}receipt_${row}[Order number].pdf
    Store the receipt as a PDF file    ${pdf_path}

Export the receipt as pdf
    ${pdf}=    Store the receipt as a PDF file    ${row}[Order number]Minimal task
    Log    Done.